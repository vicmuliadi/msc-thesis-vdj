...
Downstream analysis of spatial GEX data using the 'semla' package
...
```{r plot seurat clusters, fig.width=12}
MapLabels(se, column_name = "seurat_clusters", 
          image_use = "raw", pt_size = 1.3) +
  plot_layout(guides = "collect") &
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 3), ncol = 2))

ggsave("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD_clusters_def.png")

# Plot clusters in split view
MapLabels(se, column_name = "seurat_clusters", pt_size = 1.3,
          section_number = 1, split_labels = TRUE, ncol = 4)

ggsave("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD1_split_clusters_def.png", width = 12, units = "in")

MapLabels(se, column_name = "seurat_clusters", pt_size = 1.3,
          section_number = 2, split_labels = TRUE, ncol = 4)

ggsave("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD2_split_clusters_def.png", width = 12, units = "in")

```


```{r differentially expressed genes in clusters}
de.markers <- FindAllMarkers(se, logfc.threshold = 1, only.pos = TRUE, return.thresh = 0.01)

head(de.markers)

write_csv(de.markers, "C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD_clusters_markers_def.csv")
...
# grab the name of the first 10 marker genes for each cluster
de.markers.ten <- de.markers %>%
  group_by(cluster) %>%
  slice_head(n = 10)

write_csv(de.markers.ten, "C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD_clusters_10_markers_def.csv")
...
for (i in 1:nrow(de.markers.one)) {
  p <- MapFeatures(se, features = de.markers.one$gene[i], image_use = "raw", scale_alpha = T, colors = col_scale_genes, pt_size = 1.5, pt_alpha = 0.5)
  print(p)
  # ggsave(paste0("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/", "BCSA3_tumD_cluster", de.markers.one$cluster[i], "_marker_def.png"))
}
...
```{r TLS genes seurat clusters, fig.width=8}
tls.genes <- c("TRBC2", "MS4A1", "LTB", "LAMP3", "CXCR3", "CXCL13", "CCR7", "CCL19")

DotPlot(se, features = tls.genes) + xlab("TLS genes") + ylab("Seurat cluster")

ggsave("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/clusters/BCSA3_tumD_TLS_genes_def.png", width = 8, units = "in")
...
```{r NMF gene contributions, fig.width=10}
for (i in 1:7) {
...
```{r plot NMF gene loadings with colour scheme}
nmf.colours <- c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#66A61E", "#E6AB02", "#A6761D")

for (i in 1:7) {
  p <- PlotFeatureLoadings(se.nmf, 
                    dims = i, 
                    reduction = "nmf", 
                    nfeatures = 30,
                    mode = "dotplot", 
...
```{r feature loadings heatmap, fig.height=10}
PlotFeatureLoadings(se, 
                    dims = 1:7, 
                    reduction = "nmf", 
                    nfeatures = 20, 
                    mode = "heatmap", 
...
```{r plot all NMF, fig.width=10}
selected_factors <- c(1:7)
...
MapMultipleFeatures(se.nmf, 
                    features = paste0("NMF_", selected_factors), 
                    colors = factor_colors, 
                    image_use = "raw", 
                    override_plot_dims = TRUE, 
                    pt_size = 2.5) + plot_layout(guides = "collect")
```
...
```{r plot genes using raw counts, fig.width=10}
col_scale_genes <- paletteer::paletteer_c("grDevices::Oslo", 30)

# slot = "counts"
MapFeatures(se, features = "MS4A1", image_use = "raw", pt_size = 1.5, colors = col_scale_genes, scale_alpha = T, slot = "counts")
MapFeatures(se, features = "LAMP3", image_use = "raw", pt_size = 1.5, colors = col_scale_genes, scale_alpha = T, slot = "counts")
```
...
for (gene in marker.genes) {
  p <- MapFeatures(se.nmf, features = gene, image_use = "raw", pt_size = 1.0, slot = "data", arrange_features = "row", colors = col_scale_genes, scale_alpha = TRUE, pt_stroke = 0.5) & theme(legend.position = "right", legend.text = element_text(angle = 0, hjust = 0))
  print(p)
  ggsave(filename = paste0("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/gene_plots/","BCSA3_regD_", gene, ".png"), plot = p, width = 10, units = "in")
}
...
```{r load analysed spatial GEX data}
se <- read_rds("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/BCSA3_tumD_analysis.rds")
...
# Check genes used for cell type mapping
...
# get cell type names
DefaultAssay(se.nmf) <- "celltypeprops.major"
cell.type.major <- rownames(se.nmf)

# get gene contributions
ctype.load <- as.data.frame(Loadings(se.nmf, reduction = "nnls"))
...
cell.type.minor <- rownames(se.nmf)
...
for (ctype in cell.type.minor) {
  p <- MapFeatures(se.nmf, features = ctype, colors = viridis::magma(n = 11, direction = -1), pt_size = 2.5, override_plot_dims = T)
  print(p)
}
...
```{r cell type correlation, fig.height=10}
cell.types <- rownames(se)

cor_matrix <- FetchData(se, cell.types) |> 
...
```{r NMF using cell type proportions, fig.height=10}
nmf_data <- FetchData(se, cell.types) |> 
  RcppML::nmf(k = 10, verbose = F)

nmf_data_h <- nmf_data@h |> as.data.frame()
rownames(nmf_data_h) <- paste0("Factor_", 1:10)
colnames(nmf_data_h) <- cell.types
...
nmf_data_h_df <- nmf_data_h |> 
  tidyr::pivot_longer(cols = all_of(cell.types), 
                      names_to = "Cell", 
                      values_to = "Weight")
...
```{r plot cell type proportions, fig.width=12}

for (ctype in cell.type.major) {
  p <- MapFeatures(se.nmf, features = ctype, colors = viridis::magma(n = 11, direction = -1), pt_size = 2.5, override_plot_dims = T) & theme(legend.position = "right", legend.text = element_text(angle = 0, hjust = 0))
  print(p)
}
...
nmf.se <- Embeddings(se.nmf, reduction = "nmf")
ctype.se <- FetchData(se.nmf, paste0("celltypepropsmajor_", cell.type.major))
...
# se.nmf <- RenameAssays(se.nmf, celltypeprops = "celltypeprops.major")
# se.nmf <- RenameAssays(se.nmf, celltypeprops = "celltypeprops.minor")
se.nmf <- RenameAssays(se.nmf, celltypeprops = "celltypeprops.subset")

```

```{r cell type minor mapping correlation with NMF, fig.width=10}
nmf.se <- Embeddings(se.nmf, reduction = "nmf")
ctype.se <- FetchData(se.nmf, paste0("celltypepropsminor_", cell.type.minor))
...
```{r cell types subset}
DefaultAssay(se.nmf) <- "celltypeprops.subset"
cell.type.subset <- rownames(se.nmf)
...
nmf.se <- Embeddings(se.nmf, reduction = "nmf")
ctype.se <- FetchData(se.nmf, paste0("celltypepropssubset_", cell.type.subset))
...
```{r color palette for cell type annotation (?)}
mycolors <- RColorBrewer::brewer.pal(length(unique(ctype.annotation$`cell type`)), "Set3")
names(mycolors) <- unique(ctype.annotation$`cell type`)
mycolors <- list(category = mycolors)
```



# Seurat clusters analysis

```{r cell type subsets in seurat clusters, fig.width=25}
DotPlot(se.nmf, features = cell.type.subset, scale.max = 75) + theme(axis.text.x = element_text(angle=45, vjust = 1, hjust = 1)) + ylab("Seurat clusters")

```


```{r seurat clusters vs TLS cell types (subset), fig.width=10}
DotPlot(se.nmf, features = c("T-cells-c3-CD4+-Tfh-CXCL13", "Myeloid-c0-DC-LAMP3", "T-cells-c0-CD4+-CCR7")) + ylab("Seurat clusters")
```

```{r cell type (major) in seurat clusters}
DefaultAssay(se.nmf) <- "celltypeprops.major"
cell.type.major <- rownames(se.nmf)
DotPlot(se.nmf, features = cell.type.major) + theme(axis.text.x = element_text(angle=45, vjust = 1, hjust = 1)) + ylab("Seurat clusters")

```

```{r cell type (major) in seurat clusters, fig.width=15}
DefaultAssay(se.nmf) <- "celltypeprops.minor"
cell.type.minor <- rownames(se.nmf)
DotPlot(se.nmf, features = cell.type.minor, scale.max = 75) + theme(axis.text.x = element_text(angle=45, vjust = 1, hjust = 1)) + ylab("Seurat clusters")

```

# Plot number of unique clones in clusters (Seurat / NMF)

```{r plot unique clones in each seurat cluster}
# code from Kim:

# Idents(se2) <- "seurat_clusters"
# VlnPlot(se2, features = c("nFeature_LRIG"), ncol = 1, pt.size = 0.000001, cols = col_vector, log=T) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5,size = 12), axis.text.y = element_text(size = 16)) +NoLegend() 
# ggsave("BC_P2_Violinplot_unique_IGreceptors_clusters.png")

```

```{r plot unique clones in NMF, fig.height=10}
# DefaultAssay(se.nmf) <- "Clone"

# df.se <- GetAssayData(se.nmf)

# create new Assay with only IG clones
# df.se.ig <- df.se[grepl("IG.?", rownames(df.se)), ]
# se.nmf[["IGChain"]] <- CreateAssayObject(df.se.ig)

# for each NMF, take spots with highest activity (90th percentile)
nmf_1.spots <- names(se.nmf$nmf_1[se.nmf$nmf_1 == "1"])
nmf_1.spots <- nmf_1.spots[!is.na(nmf_1.spots)]
nmf_1.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_1.spots)

nmf_2.spots <- names(se.nmf$nmf_2[se.nmf$nmf_2 == "2"])
nmf_2.spots <- nmf_2.spots[!is.na(nmf_2.spots)]
nmf_2.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_2.spots)

nmf_3.spots <- names(se.nmf$nmf_3[se.nmf$nmf_3 == "3"])
nmf_3.spots <- nmf_3.spots[!is.na(nmf_3.spots)]
nmf_3.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_3.spots)

nmf_4.spots <- names(se.nmf$nmf_4[se.nmf$nmf_4 == "4"])
nmf_4.spots <- nmf_4.spots[!is.na(nmf_4.spots)]
nmf_4.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_4.spots)

nmf_5.spots <- names(se.nmf$nmf_5[se.nmf$nmf_5 == "5"])
nmf_5.spots <- nmf_5.spots[!is.na(nmf_5.spots)]
nmf_5.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_5.spots)

nmf_6.spots <- names(se.nmf$nmf_6[se.nmf$nmf_6 == "6"])
nmf_6.spots <- nmf_6.spots[!is.na(nmf_6.spots)]
nmf_6.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_6.spots)

nmf_7.spots <- names(se.nmf$nmf_7[se.nmf$nmf_7 == "7"])
nmf_7.spots <- nmf_7.spots[!is.na(nmf_7.spots)]
nmf_7.nclones <- FetchData(object = se.nmf, vars = "nFeature_IGChain", cells = nmf_7.spots)
...
```{r NMF_1 as cluster}
# load NMF embeddings as dataframe
nmf.df <- as.data.frame(nmf.se)

# 75th percentile
nmf1.q75 <- quantile(nmf.df$NMF_1, probs = 0.75)
nmf1.q75 <- unname(nmf1.q75)

se.nmf$nmf_1 <- ifelse(nmf.se[, 1] >= nmf1.q75, "1", NA)

nmf1.q75.pl <- MapLabels(se.nmf, column_name = "nmf_1", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))

# 90th percentile
nmf1.q90 <- quantile(nmf.df$NMF_1, probs = 0.90)
nmf1.q90 <- unname(nmf1.q90)

se.nmf$nmf_1 <- ifelse(nmf.se[, 1] >= nmf1.q90, "1", NA)

nmf1.q90.pl <- MapLabels(se.nmf, column_name = "nmf_1", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
```{r NMF_2 as cluster}
# 75th percentile
nmf2.q75 <- quantile(nmf.df$NMF_2, probs = 0.75)
nmf2.q75 <- unname(nmf2.q75)

se.nmf$nmf_2 <- ifelse(nmf.se[, 2] >= nmf2.q75, "2", NA)

nmf2.q75.pl <- MapLabels(se.nmf, column_name = "nmf_2", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
se.nmf$nmf_2 <- ifelse(nmf.se[, 2] >= nmf2.q90, "2", NA)

nmf2.q90.pl <- MapLabels(se.nmf, column_name = "nmf_2", override_plot_dims = TRUE, 
...
```{r radial distance NMF_2 q75}
DefaultAssay(se.nmf) <- "Clone"

se.nmf <- RadialDistance(se.nmf, column_name = "nmf_2", 
                     selected_groups = "2", convert_to_microns = TRUE)

se.nmf$r_dist_2_sqrt <- sign(se.nmf$r_dist_2)*sqrt(abs(se.nmf$r_dist_2))
MapFeatures(se.nmf, features = "r_dist_2_sqrt", center_zero = TRUE, pt_size = 2,
            colors = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(),
            override_plot_dims = TRUE)

```

radial distances are measured from cluster border
positive distances: further away from cluster border
negative distances: closer to / within cluster border

```{r radial distance of IGK clones from NMF_2 q75}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igk.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
...
```{r radial distance of IGL clones from NMF_2 q75}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igl.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
...
```{r radial distance of IGHG clones from NMF_2 q75}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighg.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(ighg.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_2, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")
...
```{r radial distance of IGHA clones from NMF_2 q75}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igha.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(igha.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_2, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")
...
```{r radial distance of IGHM clones from NMF_2 q75}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighm.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(ighm.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_2, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")
...
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = lchain)) |> 
  filter(r_dist_2 < 2e3) |> 
...
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = hchain)) |> 
  filter(r_dist_2 < 2e3) |> 
...
```{r radial distance of IGHG clones from NMF_2 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighg.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(ighg.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_2, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed") +
  xlab("Distance from F2 border (µm)")

```

```{r radial distance of IGHA clones from NMF_2 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igha.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(igha.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHM clones from NMF_2 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighm.clones)) |> 
  filter(r_dist_2 < 2e3) |> 
  pivot_longer(all_of(ighm.clones), names_to = "variable", values_to = "value") |> 
...
```{r NMF_3 as cluster}
# 75th percentile
nmf3.q75 <- quantile(nmf.df$NMF_3, probs = 0.75)
nmf3.q75 <- unname(nmf3.q75)

se.nmf$nmf_3 <- ifelse(nmf.se[, 3] >= nmf3.q75, "3", NA)

nmf3.q75.pl <- MapLabels(se.nmf, column_name = "nmf_3", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
se.nmf$nmf_3 <- ifelse(nmf.se[, 3] >= nmf3.q90, "3", NA)

nmf3.q90.pl <- MapLabels(se.nmf, column_name = "nmf_3", override_plot_dims = TRUE, 
...
se.nmf <- RadialDistance(se.nmf, column_name = "nmf_3", 
                     selected_groups = "3", convert_to_microns = TRUE)

se.nmf$r_dist_3_sqrt <- sign(se.nmf$r_dist_3)*sqrt(abs(se.nmf$r_dist_3))
MapFeatures(se.nmf, features = "r_dist_3_sqrt", center_zero = TRUE, pt_size = 2,
            colors = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(),
            override_plot_dims = TRUE)
...
```{r radial distance of IGK clones from NMF_3 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igk.clones)) |> 
  filter(r_dist_3 < 2e3) |> 
  pivot_longer(all_of(igk.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGL clones from NMF_3 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igl.clones)) |> 
  filter(r_dist_3 < 2e3) |> 
  pivot_longer(all_of(igl.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHG clones from NMF_3 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighg.clones)) |> 
  filter(r_dist_3 < 2e3) |> 
  pivot_longer(all_of(ighg.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHA clones from NMF_3 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igha.clones)) |> 
  filter(r_dist_3 < 2e3) |> 
  pivot_longer(all_of(igha.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHM clones from NMF_3 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighm.clones)) |> 
  filter(r_dist_3 < 2e3) |> 
  pivot_longer(all_of(ighm.clones), names_to = "variable", values_to = "value") |> 
...
```{r NMF_4 as cluster}
# 75th percentile
nmf4.q75 <- quantile(nmf.df$NMF_4, probs = 0.75)
nmf4.q75 <- unname(nmf4.q75)

se.nmf$nmf_4 <- ifelse(nmf.se[, 4] >= nmf4.q75, "4", NA)

nmf4.q75.pl <- MapLabels(se.nmf, column_name = "nmf_4", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))

# 90th percentile
nmf4.q90 <- quantile(nmf.df$NMF_4, probs = 0.90)
nmf4.q90 <- unname(nmf4.q90)

se.nmf$nmf_4 <- ifelse(nmf.se[, 4] >= nmf4.q90, "4", NA)

nmf4.q90.pl <- MapLabels(se.nmf, column_name = "nmf_4", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
```{r radial distance NMF_4 q90}
DefaultAssay(se.nmf) <- "Clone"

se.nmf <- RadialDistance(se.nmf, column_name = "nmf_4", 
                     selected_groups = "4", convert_to_microns = TRUE)

se.nmf$r_dist_4_sqrt <- sign(se.nmf$r_dist_4)*sqrt(abs(se.nmf$r_dist_4))
MapFeatures(se.nmf, features = "r_dist_4_sqrt", center_zero = TRUE, pt_size = 2,
            colors = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(),
            override_plot_dims = TRUE)
...
```{r radial distance of IGK clones from NMF_4 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igk.clones)) |> 
  filter(r_dist_4 < 2e3) |> 
  pivot_longer(all_of(igk.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_4, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")

```

```{r radial distance of IGL clones from NMF_4 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igl.clones)) |> 
  filter(r_dist_4 < 2e3) |> 
  pivot_longer(all_of(igl.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHG clones from NMF_4 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighg.clones)) |> 
  filter(r_dist_4 < 2e3) |> 
  pivot_longer(all_of(ighg.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHA clones from NMF_4 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igha.clones)) |> 
  filter(r_dist_4 < 2e3) |> 
  pivot_longer(all_of(igha.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHM clones from NMF_4 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighm.clones)) |> 
  filter(r_dist_4 < 2e3) |> 
  pivot_longer(all_of(ighm.clones), names_to = "variable", values_to = "value") |> 
...
```{r NMF_5 as cluster}
# 90th percentile
nmf5.q90 <- quantile(nmf.df$NMF_5, probs = 0.90)
nmf5.q90 <- unname(nmf5.q90)

se.nmf$nmf_5 <- ifelse(nmf.se[, 5] >= nmf5.q90, "5", NA)

nmf5.q90.pl <- MapLabels(se.nmf, column_name = "nmf_5", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
```{r NMF_6 as cluster}
# 90th percentile
nmf6.q90 <- quantile(nmf.df$NMF_6, probs = 0.90)
nmf6.q90 <- unname(nmf6.q90)

se.nmf$nmf_6 <- ifelse(nmf.se[, 6] >= nmf6.q90, "6", NA)

nmf6.q90.pl <- MapLabels(se.nmf, column_name = "nmf_6", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
```{r NMF_7 as cluster}
# 75th percentile
nmf7.q75 <- quantile(nmf.df$NMF_7, probs = 0.75)
nmf7.q75 <- unname(nmf7.q75)

se.nmf$nmf_7 <- ifelse(nmf.se[, 7] >= nmf7.q75, "7", NA)

nmf7.q75.pl <- MapLabels(se.nmf, column_name = "nmf_7", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))

# 90th percentile
nmf7.q90 <- quantile(nmf.df$NMF_7, probs = 0.90)
nmf7.q90 <- unname(nmf7.q90)

se.nmf$nmf_7 <- ifelse(nmf.se[, 7] >= nmf7.q90, "7", NA)

nmf7.q90.pl <- MapLabels(se.nmf, column_name = "nmf_7", override_plot_dims = TRUE, 
                         image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
...
```{r radial distance NMF_7 q90}
DefaultAssay(se.nmf) <- "Clone"

se.nmf <- RadialDistance(se.nmf, column_name = "nmf_7", 
                     selected_groups = "7", convert_to_microns = TRUE)

se.nmf$r_dist_7_sqrt <- sign(se.nmf$r_dist_7)*sqrt(abs(se.nmf$r_dist_4))
MapFeatures(se.nmf, features = "r_dist_7_sqrt", center_zero = TRUE, pt_size = 2,
            colors = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(),
            override_plot_dims = TRUE)
...
```{r radial distance of IGK clones from NMF_7 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igk.clones)) |> 
  filter(r_dist_7 < 2e3) |> 
  pivot_longer(all_of(igk.clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_7, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")

```

```{r radial distance of IGL clones from NMF_7 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igl.clones)) |> 
  filter(r_dist_7 < 2e3) |> 
  pivot_longer(all_of(igl.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHG clones from NMF_7 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighg.clones)) |> 
  filter(r_dist_7 < 2e3) |> 
  pivot_longer(all_of(ighg.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHA clones from NMF_7 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = igha.clones)) |> 
  filter(r_dist_7 < 2e3) |> 
  pivot_longer(all_of(igha.clones), names_to = "variable", values_to = "value") |> 
...
```{r radial distance of IGHM clones from NMF_7 q90}
se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = ighm.clones)) |> 
  filter(r_dist_7 < 2e3) |> 
  pivot_longer(all_of(ighm.clones), names_to = "variable", values_to = "value") |> 
...
# Radial distances with Seurat clusters (not used )

```{r seurat cluster 5}
se.nmf$cluster_5 <- ifelse(se.nmf$seurat_clusters %in% "5", "5", NA)
MapLabels(se.nmf, column_name = "cluster_5", override_plot_dims = TRUE, 
          image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
```

```{r radial distance cluster 5}
DefaultAssay(se.nmf) <- "Clone"

se.nmf <- RadialDistance(se.nmf, column_name = "seurat_clusters", 
                     selected_groups = "5", convert_to_microns = TRUE)

# se.nmf$r_dist_5_sqrt <- sign(se.nmf$r_dist_5)*sqrt(abs(se.nmf$r_dist_5))
# MapFeatures(se.nmf, features = "r_dist_5_sqrt", center_zero = TRUE, pt_size = 2, 
#             colors = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(),
#             override_plot_dims = TRUE)
...
```{r radial distance of clones from cluster 5}
sel_clones <- c("IGLclone8", "IGHGclone58", "IGKclone17")

se.nmf[[]] |> 
  bind_cols(FetchData(se.nmf, vars = sel_clones)) |> 
  filter(r_dist_5 < 1e3) |> 
  pivot_longer(all_of(sel_clones), names_to = "variable", values_to = "value") |> 
  ggplot(aes(r_dist_5, value, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed")

```

```{r plot distances of clones cluster 5, fig.width=10}
# colour scheme
col_scale_genes <- paletteer::paletteer_c("grDevices::Oslo", 30)

MapFeatures(SubsetSTData(se.nmf, expression = r_dist_5 < 1e3), 
            features = sel_clones, override_plot_dims = TRUE, scale_alpha = TRUE,
            image_use = "raw", pt_size = 2, colors = col_scale_genes)
...
```{r seurat cluster 3}
se.nmf$cluster_3 <- ifelse(se.nmf$seurat_clusters %in% "3", "3", NA)
MapLabels(se.nmf, column_name = "cluster_3", override_plot_dims = TRUE, 
          image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 2))
```

```{r radial distance cluster 3}
# DefaultAssay(se.nmf) <- "Clone"
...
```{r radial distance of clones from cluster 3}
# sel_clones <- c("IGLclone8", "IGHGclone58", "IGKclone17")
...
MapFeatures(SubsetSTData(se.nmf, expression = r_dist_3 < 1e3), 
            features = sel_clones, override_plot_dims = TRUE, scale_alpha = TRUE,
            image_use = "raw", pt_size = 2, colors = col_scale_genes)
...
```{r save seurat}
write_rds(se.nmf, "C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/BCSA3_tumD_analysis_ctype.rds")

se.nmf <- read_rds("C:/Users/victo/Downloads/spatialVDJ/scripts/sample_analysis/BCSA3_tumD_analysis_ctype.rds")
```
